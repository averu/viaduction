...
要件 (`RC-XXX` または `REQ-XXX`) の **読み取り専用レビュー** Skill。
**書き換えは行わず、指摘のみを返す**。

## いつ使うか

- `refine-requirements` で `RC-XXX` を起票した直後
- `specify-requirements` の前後
- ユーザが「要件レビューして」「BLOCKER 確認して」と言ったとき
