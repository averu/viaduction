---
description: 基本設計を入力に、詳細設計 (docs/20-detail-design/) を detail-design-architect に生成させる。SCR/API/DB それぞれ 1 ID 1 ファイル。
...
`detail-design-architect` Subagent を呼んで、`docs/20-detail-design/{screens,apis,db}/` 配下の詳細設計を生成・更新します。
...
5. 人間に「`/design-review` を実行しますか？」と確認。
...
- `npx tsx scripts/validate-traceability.ts` の終了コードが 0 または 2

## 関連コマンド

- 前フェーズ: `/basic-design`
- 次フェーズ: `/task-breakdown`
