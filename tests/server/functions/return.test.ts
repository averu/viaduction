// TEST-018 — returnProposal server function
// REQ-003 / REQ-006 / REQ-010 / REQ-011 / NFR-002 / NFR-003 / NFR-005 / NFR-006 /
// NFR-007 / API-005 / DB-003 / DB-004 / UC-005 / UC-013 / UC-014 /
...
import {
  ...
  type ReturnInput,
  type ReturnResult,
} from '../../../src/server/functions/return'
import {
...
function makeDeps(proposalsInitial: ReadonlyArray<Proposal>): DepsBundle {
  ...
  const deps: ReturnDeps = { proposals, audit }
...
describe('REQ-003 / API-005 / TEST-018: returnProposal happy path', () => {
  beforeEach(() => {
    vi.useFakeTimers()
  ...
  it('REQ-003 / TEST-018: reviewer returns in_review proposal → status=returned, assignee_id=null, approved_at=null, version+1, AuditLog appended', async () => {
    const initial = makeInReviewProposal({ version: 1 })
    const { deps, proposals, audit } = makeDeps([initial])

    const result = await returnProposal(reviewerViewer, PROPOSAL_ID, VALID_INPUT, deps)
    ...
    expect(entry?.action).toBe('return')
    ...
    expect(entry?.after_status).toBe('returned')
  ...
  it('REQ-003 / TEST-018: admin returns in_review proposal → status=returned, AuditLog actor_role=admin', async () => {
    const initial = makeInReviewProposal()
    const { deps, proposals, audit } = makeDeps([initial])

    const result = await returnProposal(adminViewer, PROPOSAL_ID, VALID_INPUT, deps)
  ...
  it('API-005 / TEST-018: actor_role joins multiple roles with comma (DB-004 m-02)', async () => {
    ...
    await returnProposal(multiRoleViewer, PROPOSAL_ID, VALID_INPUT, deps)

    const logs = await audit.list()
    expect(logs[0]?.actor_role).toBe('reviewer,admin')
  })

  it('BR-REVIEW-01 / TEST-018: reason is trimmed before persisting in AuditLog', async () => {
    const initial = makeInReviewProposal()
    const { deps, audit } = makeDeps([initial])

    await returnProposal(
      reviewerViewer,
      PROPOSAL_ID,
      { reason: '   個人名が含まれています   ', expected_version: 1 },
      deps,
    )

    const logs = await audit.list()
    expect(logs[0]?.reason).toBe('個人名が含まれています')
  })

  it('API-005 / TEST-018: reviewer (not the original assignee) can return → B-3 撤回（assignee 一致は要求しない）', async () => {
    // initial の assignee_id=reviewer-1 だが、別の reviewer-2 が return できる
    const initial = makeInReviewProposal({ assignee_id: reviewerViewer.user_id })
    const { deps, proposals } = makeDeps([initial])

    const result = await returnProposal(otherReviewerViewer, PROPOSAL_ID, VALID_INPUT, deps)
    expect(result.status).toBe('returned')
  ...
  it('API-005 / TEST-018: admin can return in_review private proposal (B-3 撤回 + Q-016 はここでは無関係)', async () => {
  ...
  it('API-005 / TEST-018: reviewer can return in_review private proposal (review.return は visibility を参照しない)', async () => {
    // start_review は reviewer + private を 404 にするが (Q-016 暫定)、
    // 一度 in_review に到達した proposal の判定 (approve / return / reject) は
    // authorize の review.return 分岐で resource を参照しないため、
    // reviewer + private + in_review でも通過する（B-3 撤回 + API-005 §認可）。
    const initial = makeProposal(
      { author_id: ownerViewer.user_id, status: 'in_review', visibility: 'private' },
      { id: PROPOSAL_ID, version: 1, assignee_id: adminViewer.user_id },
    )
  ...
  it('API-005 / TEST-018: returned proposal preserves submitted_at (REQ-006 で再提出時に上書き)', async () => {
    // returned 状態の submitted_at は initial submit 時刻のまま据え置き。
    // API-009 resubmit で returned → submitted のときに上書き更新される（DB-003 §関連 API）。
    const initial = makeInReviewProposal({ submitted_at: 1_000 })
    const { deps, proposals } = makeDeps([initial])

    await returnProposal(reviewerViewer, PROPOSAL_ID, VALID_INPUT, deps)
...
describe('API-005 / NFR-003 / BR-AUTHZ-03 / TEST-018: authorization', () => {
  it('API-005 / TEST-018: guest (viewer === null) → AuthorizationError(401, not_authenticated), no AuditLog', async () => {
    const initial = makeInReviewProposal()
    const { deps, audit, proposals } = makeDeps([initial])

    let thrown: unknown
    try {
      await returnProposal(null, PROPOSAL_ID, VALID_INPUT, deps)
    } catch (e) {
      thrown = e
    }
  ...
  it('API-005 / TEST-018: nonexistent proposal id (authenticated reviewer) → 404 not_owner_resource, no AuditLog', async () => {
    const { deps, audit } = makeDeps([])

    let thrown: unknown
    try {
      await returnProposal(reviewerViewer, 'missing-id', VALID_INPUT, deps)
    } catch (e) {
      thrown = e
    }
  ...
  it('API-005 / TEST-018: nonexistent proposal id (guest) → 401 not_authenticated', async () => {
    const { deps, audit } = makeDeps([])

    await expect(
      returnProposal(null, 'missing-id', VALID_INPUT, deps),
  ...
  it('API-005 / TEST-018: user role → AuthorizationError(404, insufficient_role)', async () => {
    const initial = makeInReviewProposal()
    const { deps, audit } = makeDeps([initial])

    await expect(
      returnProposal(ownerViewer, PROPOSAL_ID, VALID_INPUT, deps),
    ).rejects.toMatchObject({
      name: 'AuthorizationError',
      httpStatus: 404,
  ...
  it('API-005 / TEST-018: auditor role → AuthorizationError(404, insufficient_role)', async () => {
    const initial = makeInReviewProposal()
    const { deps, audit } = makeDeps([initial])

    await expect(
      returnProposal(auditorViewer, PROPOSAL_ID, VALID_INPUT, deps),
    ).rejects.toMatchObject({
      name: 'AuthorizationError',
      httpStatus: 404,
      reason: 'insufficient_role',
...
describe('BR-REVIEW-01 / API-005 / TEST-018: reason validation', () => {
  it('BR-REVIEW-01 / TEST-018: reason empty string → ReturnValidationError(field=reason), no AuditLog', async () => {
    const initial = makeInReviewProposal()
    const { deps, audit, proposals } = makeDeps([initial])
    ...
    expect(thrown).toBeInstanceOf(ReturnValidationError)
    if (thrown instanceof ReturnValidationError) {
  ...
  it('BR-REVIEW-01 / TEST-018: reason whitespace-only → ReturnValidationError(field=reason), no AuditLog', async () => {
    const initial = makeInReviewProposal()
    const { deps, audit } = makeDeps([initial])
  ...
  it('BR-REVIEW-01 / TEST-018: reason 4001 chars → ReturnValidationError(field=reason), no AuditLog', async () => {
    const initial = makeInReviewProposal()
    const { deps, audit } = makeDeps([initial])
  ...
  it('BR-REVIEW-01 / TEST-018: reason exactly 4000 chars → success', async () => {
    const initial = makeInReviewProposal()
    const { deps, audit } = makeDeps([initial])
  ...
  it('API-005 / TEST-018: reason non-string → ReturnValidationError(field=reason)', async () => {
    const initial = makeInReviewProposal()
    const { deps, audit } = makeDeps([initial])
    const bad = { reason: 123, expected_version: 1 } as unknown as ReturnInput

    await expect(
      returnProposal(reviewerViewer, PROPOSAL_ID, bad, deps),
    ).rejects.toMatchObject({
      field: 'reason',
    })
    expect(await audit.list()).toHaveLength(0)
...
describe('API-005 / BR-REVIEW-02 / TEST-018: expected_version validation', () => {
  it('API-005 / TEST-018: expected_version negative → ReturnValidationError(field=expected_version)', async () => {
    const initial = makeInReviewProposal()
    const { deps, audit } = makeDeps([initial])
  ...
  it('API-005 / TEST-018: expected_version non-integer → ReturnValidationError(field=expected_version)', async () => {
    const initial = makeInReviewProposal()
    const { deps, audit } = makeDeps([initial])
  ...
  it('API-005 / TEST-018: expected_version not a number → ReturnValidationError(field=expected_version)', async () => {
    const initial = makeInReviewProposal()
    const { deps, audit } = makeDeps([initial])
    const bad = {
      reason: 'reason',
      expected_version: 'one',
    } as unknown as ReturnInput
...
// ---------------------------------------------------------------------------
// 5. status != in_review (DB-003 不変条件 1 / API-005 §422)
// ---------------------------------------------------------------------------

describe('API-005 / DB-003 / TEST-018: state guard (only in_review can be returned)', () => {
  ...
  for (const status of NON_IN_REVIEW_STATUSES) {
    it(`DB-003 / TEST-018: status='${status}' → ReturnStateError(reason=status_not_in_review), no AuditLog`, async () => {
      ...
      expect(thrown).toBeInstanceOf(ReturnStateError)
      if (thrown instanceof ReturnStateError) {
      ...
      // 「return が approved_at を上書きしていない」ことを確認する。
...
describe('API-005 / BR-REVIEW-02 / TEST-018: optimistic lock', () => {
  it('API-005 / TEST-018: expected_version mismatch → ProposalLockError, no AuditLog, no state change', async () => {
  ...
  it('API-005 / TEST-018: simultaneous return by two reviewers → only first succeeds, second gets ReturnStateError (post-transition)', async () => {
    const initial = makeInReviewProposal({ version: 1 })
    const { deps, audit } = makeDeps([initial])

    // reviewer-1 が先に成功
    const first = await returnProposal(reviewerViewer, PROPOSAL_ID, VALID_INPUT, deps)
    expect(first.status).toBe('returned')

    // reviewer-2 が同じ expected_version=1 で呼ぶと、現バージョンは 2 / 状態は returned
    // のため status guard で 422 に倒れる（CAS ではなく status 検査が先に失敗する経路）。
    await expect(
      returnProposal(otherReviewerViewer, PROPOSAL_ID, VALID_INPUT, deps),
    ).rejects.toBeInstanceOf(ReturnStateError)
  ...
  it('API-005 / TEST-018: stale expected_version on still-in_review proposal → ProposalLockError (409 path)', async () => {
    // proposals の version が 7 まで進んだ状態で、status は in_review のまま
    // （別経路で version だけ進んだ想定）。reviewer が古い version=6 を指定して
    // return すると CAS 失敗で ProposalLockError → 呼び出し側 wrapper が 409 にマップ。
...
// ---------------------------------------------------------------------------
// 7. 書き込み順序 (API-005 §書き込み順序)
// ---------------------------------------------------------------------------

describe('API-005 / TEST-018: write ordering', () => {
  it('API-005 / TEST-018: spy invocation order is findById → updateWithLock → audit.append', async () => {
    const initial = makeInReviewProposal()
    const { deps, proposals, audit } = makeDeps([initial])
  ...
  it('API-005 / TEST-018: updateWithLock receives status=returned, assignee_id=null, NO approved_at key', async () => {
    const initial = makeInReviewProposal()
    const { deps, proposals } = makeDeps([initial])
    ...
    if (callArgs !== undefined) {
      ...
      // approved_at は patch に含めない（DB-003 §approved_at「returned 経路では NULL のまま」）。
      // patch のキー集合に approved_at が含まれないことを確認する。
      expect(Object.prototype.hasOwnProperty.call(patch, 'approved_at')).toBe(false)
    }
  })

  it('API-005 / TEST-018: when validation fails, no write paths are called (BR-AUDIT-03)', async () => {
    const initial = makeInReviewProposal()
    const { deps, proposals, audit } = makeDeps([initial])
  ...
  it('API-005 / TEST-018: when authorize fails, no write paths are called (BR-AUDIT-03)', async () => {
    const initial = makeInReviewProposal()
    const { deps, proposals, audit } = makeDeps([initial])
  ...
  it('API-005 / TEST-018: when status guard fails, no write paths are called (BR-AUDIT-03)', async () => {
    ...
    await expect(
      returnProposal(reviewerViewer, PROPOSAL_ID, VALID_INPUT, deps),
    ).rejects.toBeInstanceOf(ReturnStateError)
  ...
  it('API-005 / TEST-018: when optimistic lock fails, AuditLog is NOT appended (BR-AUDIT-03)', async () => {
    const initial = makeInReviewProposal({ version: 5 })
    const { deps, audit } = makeDeps([initial])
...
describe('NFR-005 / API-005 / TEST-018: no console / logger side effects', () => {
  it('NFR-005 / TEST-018: no console.* calls during success path', async () => {
  ...
  it('NFR-005 / TEST-018: injected logger is never called by returnProposal itself', async () => {
    const initial = makeInReviewProposal()
    ...
    await returnProposal(reviewerViewer, PROPOSAL_ID, VALID_INPUT, {
      proposals,
      audit,
      logger,
    })
...
describe('API-005 / TEST-018: result type narrowing', () => {
  it('API-005 / TEST-018: result.status is the literal "returned" and version is a finite number', async () => {
    const initial = makeInReviewProposal()
    const { deps } = makeDeps([initial])

    const result: ReturnResult = await returnProposal(
      reviewerViewer,
      PROPOSAL_ID,
      VALID_INPUT,
      deps,
    ...
    // 型レベルチェック: 'returned' リテラル以外を割り当てると TS2322 が出る。
    const statusLiteral: 'returned' = result.status
    expect(statusLiteral).toBe('returned')
