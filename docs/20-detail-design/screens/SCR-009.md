...
`reviewer` または `admin` が `submitted` / `in_review` の提案詳細を確認し、判定操作（担当開始 / 承認 / 差し戻し / 却下）を実行する画面（UC-003〜UC-006）。AuditLog 履歴抜粋を表示してレビュアーの判断補助とする（API-015 と整合）。

> **公開操作 (UC-007) は本画面に含めない（M-9 整理）**: API-015 が `submitted` / `in_review` のみを返す設計に合わせ、`approved` 投稿の公開操作は本画面ではなく **`SCR-013` 公開操作画面（API-021 で proposal 取得 → API-007 で公開）** に集約する。
...
| ステータスバッジ | enum（`Badge`、submitted / in_review） | yes | API-015 から取得 | 表示のみ。**approved は API-015 が返さない（M-9 整理、SCR-013 へ）** | API-015 |
| visibility バッジ | enum（`Badge`） | yes | API-015 から取得 | reviewer は `public` / `internal`、admin は `private` も | API-015 |
| 投稿者表示名 | text | yes | API-015 から取得 | 表示のみ。**表示専用扱い**（logger には出さない、DB-006 不変条件 3 / M-2 確定） | API-015 |
...
| 確認 AlertDialog（4 アクションで共通テンプレート） | `AlertDialog` | yes（判定実行時） | hidden | reason textarea + 「実行」「キャンセル」 | — |
| `expected_version` | hidden（数値） | yes（mutation 時） | API-015 から取得 | server-side で CAS 検証 → 不一致時 409 | API-003〜006 |
...
> **公開操作 (UC-007) / 取り下げ (UC-008) の入口は本画面では提供しない（M-9 整理）**: API-015 が `submitted` / `in_review` のみを返す設計と整合させ、`approved → published`（API-007）および `published → withdrawn`（API-008）はすべて **SCR-013 公開操作画面** に集約する（API-021 で proposal 取得 → API-007 / API-008）。本画面は **レビュー判定 4 種に専念**。
...
stateDiagram-v2
  ...
  loading --> notFound: 404（不存在 / role 不足 / private ブロック / status 範囲外（approved/rejected/returned/published/withdrawn 含む））
  ...
  success --> [*]: SCR-008 へリダイレクト
  unauthorized --> [*]: SCR-007 へ
  notFound --> [*]: SCR-008 へ
  error --> loading: リトライ
```

> **approved の公開導線**: 本画面で approved に到達することはない（API-015 が 404）。admin は SCR-013 から公開（M-9 整理、SCR-013 → API-021 → API-007）。
...
| ロール / status | submitted | in_review (assignee=自身) | in_review (assignee=他人) | approved / rejected / returned / published / withdrawn |
| --- | --- | --- | --- | --- |
| guest | 401 | 401 | 401 | 401 |
| user | 404 | 404 | 404 | 404 |
| reviewer | 「担当開始」表示 | 「承認」「差し戻し」「却下」表示 | 閲覧のみ（自身が担当でない旨表示） | 404（API-015 が 404 で隠蔽） |
| admin | 「担当開始」表示 | 「承認」「差し戻し」「却下」表示（admin が assignee 時） | 閲覧のみ | 404（公開操作は SCR-013 へ、M-9 整理） |
| auditor | 404 | 404 | 404 | 404 |

> 本画面の loader (API-015) は status が `submitted` / `in_review` 以外の場合 404 で隠蔽する。`approved` 投稿の公開操作は本画面では提供せず、**SCR-013 公開操作画面** に集約（M-9 整理、SCR-013 → API-021 → API-007）。

## 参照

- 上流: `docs/02-requirements/02-functional-requirements.md` の REQ-003 / REQ-004 / REQ-009 / REQ-010 / REQ-011 / REQ-015、`docs/02-requirements/03-non-functional-requirements.md` の NFR-002〜007、`docs/02-requirements/04-business-rules.md` の `BR-REVIEW-01` / `BR-REVIEW-02` / `BR-PUBLISH-01`、`docs/10-basic-design/01-system-overview.md` の UC-003〜UC-006（UC-007 公開は SCR-013 に集約、M-9 整理）、`docs/10-basic-design/03-screen-list.md`、`docs/00-discovery/07-open-questions.md` の Q-016
- 下流: `docs/20-detail-design/apis/API-015.md`、`API-003.md`、`API-004.md`、`API-005.md`、`API-006.md`、TASK-XXX（Phase 5）
