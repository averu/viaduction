...
| 提案カード：投稿者表示名 | text（読み取り専用） | no | `author_id` を表示用に解決（暫定: `author_id` を短縮形で表示、display_name 解決は Phase 5 で確定） | 表示のみ。**display_name を採用する際は表示専用扱い**（logger には出さない、DB-006 不変条件 3 / M-2 確定） | API-010 |
...
> **認証状態の検出と表示切替（M-10 整理）**: ヘッダのナビゲーション（「自分の投稿」「レビュー待ち」「監査ログ」「ログイン / ログアウト」）は **loader レスポンス + client-side の cookie 解析（HttpOnly のため client は cookie 値を直接読めないが、loader レスポンスに付随する viewer メタ情報、または別の loader 経由の認証状態確認で代替）** で切替する。loader 自体は公開バイパスで guest でも 200 を返すため、認証切れは個別 mutation 失敗（401）で検出 → guest 状態に再ロード + バナー表示の流れ。
...
| 認証切れ | guest として再 loader（公開バイパスのため自動的に `public` のみに絞られる）。**ヘッダのナビゲーション（「自分の投稿」「レビュー待ち」「監査ログ」等）の出し分けは loader レスポンス + client-side の cookie 解析結果で切替**（M-10 整理）。ログイン状態の解析は client-side で行い、cookie 失効時はナビゲーションも guest 表示にフォールバック。 | バナー「セッションが切れました。再ログインで internal も表示されます」+ `SCR-007` リンク |
