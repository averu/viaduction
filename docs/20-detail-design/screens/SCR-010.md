...
| フィルタ：action | multi-select（`Select` 複数選択、8 値: submit / start_review / approve / return / reject / resubmit / publish / withdraw） | no | （全件） | enum 検証、UI 側で CSV にエンコードして送信（`?action=submit,publish`）。**送信時は server-side で再検証** | API-016 |
| フィルタ：from（期間 from） | datetime-local（`DatePicker`） | no | （空） | datetime → Unix epoch millis 変換。**送信前に from <= to を UI 側で一次チェックし、不整合時は「適用」ボタンを disabled**（M-14 minor）。 | API-016 |
| フィルタ：to（期間 to） | datetime-local（`DatePicker`） | no | （空） | from <= to を UI 側で一次チェック（disabled 化）。**送信時は server-side でも検証**（`from > to` で 400） | API-016 |
...
| AuditLog テーブル：reason_present | boolean（`Checkbox` icon、disabled） | yes | API-016 から取得 | `true` は「あり」アイコン、`false` は「なし」。**API-016 のレスポンス `reason_present` フィールドのみを表示**（reason 本体は API-017 詳細でのみ取得、NFR-005 PII 除外、M-12 確定） | API-016 |
...
2. 主シナリオ:
   ...
   3. フィルタを変更（actor_id / action / from / to。M-1 で `target_proposal_id` フィルタは撤回）→「適用」押下 → URL に query を付与して再 loader。
...
- フォーカス順序: ヘッダ → フィルタ群（actor → action → from → to → 「適用」→「リセット」）→ テーブル各行 → ページネーション
