...
> **型の方針（NFR-002 と整合）**: SQLite 互換の `text` / `integer` のみ。`visibility` / `status` の enum は `text` + CHECK 制約で表現（`text CHECK (visibility IN ('private','internal','public'))` 等）。タイムスタンプは Unix epoch millis。
